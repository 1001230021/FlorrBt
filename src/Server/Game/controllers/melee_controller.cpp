...
void CMeleeController::OnTick(CMobBase* mob, float dt)
{
	...
	sf::Vector2f target_vel = { mob->GetFinalStats()->max_velocity * cosf(angle), mob->GetFinalStats()->max_velocity * sinf(angle) };
	sf::Vector2f delta_vel = target_vel - mob->m_Vel;
	float spu_angle = atan2f(delta_vel.x, delta_vel.y);
	mob->m_Vel += { mob->GetFinalStats()->acceleration * cosf(spu_angle) * dt, mob->GetFinalStats()->acceleration * sinf(spu_angle) * dt };
    ...
    if (CheckChance(p))
    {
        ...
        auto candidates = mob->GameWorld()->GetSpatialGrid().QueryRange(
            mob->m_Pos,
            mob->GetFinalStats()->search_range * 2.f,
                ...
                if (e)
                    if (!CheckTeam(e->m_Team, mob->m_Team)) 
                    {
                        ...
                        float mult = dynamic_cast<const CFlower*>(e) ?
                            static_cast<const CFlower*>(e)->m_FinalStats.detection_multiplier : 1.f;
