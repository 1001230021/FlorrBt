...
void CPlayerController::OnTick(CMobBase* mob, float dt)
{
    ...
    {
        sf::Vector2f target = mob->m_pos + m_move_dir * 100123.f;
...
void CPlayerController::ExecuteOperate(const ClientOperate& op, CMobBase* mob)
{
    ...
    case ClientOperate::Type::Input:
        ...
        {
            float move_x = std::clamp(static_cast<float>(*op.move_x) / 127.f, -1.f, 1.f);
            float move_y = std::clamp(static_cast<float>(*op.move_y) / 127.f, -1.f, 1.f);
    ...
    case ClientOperate::Type::AttackDefend:
        if (flower)
        {
            if (op.isAttacking.has_value()) flower->m_attacking = *op.isAttacking;
            if (op.isDefending.has_value()) flower->m_defending = *op.isDefending;
        }
        break;
